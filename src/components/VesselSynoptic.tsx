...
export function VesselSynoptic({ vessel }: { vessel: VesselState }) {
  ...
  // ROUND 124 (design-system conformance): this schematic is the only one of these
  // SVGs that uses a SCALING viewBox (720-unit space rendered at `w` px), so its text
  // and strokes were rendering ~w/720× larger + heavier than the pixel-space EGT/envelope
  // charts. Compensate by /k so both land at their true tier px / px weights, identical
  // to the refined panels. Tiers: --type-micro 10, --type-context 13 (no one-off sizes).
  const k = w > 0 ? w / VB.w : 1;
  const MICRO = 10, CONTEXT = 13;
  const mono = (px: number = MICRO): React.CSSProperties => ({ fontFamily: FONT.data, fontSize: px / k });
  const sw = (px: number) => px / k; // stroke weights at true px under the viewBox scale
    ...
    <div style={{ marginBottom: 'var(--pad-stack)' }}>
      ...
      <div ref={wrapRef}>
        <svg width={w} height={h} viewBox={`0 0 ${VB.w} ${VB.h}`} style={{ display: 'block' }}>
          ...
          <path {...layer('VesselSynoptic / hull / hull.line', 'ink/secondary 1.5px — neutral, never status', '{HULL_PATH — Figma hull replaces 1:1}')} d={HULL_PATH} fill="none" stroke={INK2} strokeWidth={sw(1.25)} />
          <path {...layer('VesselSynoptic / hull / superstructure.shape', 'surface/overlay fill · line/strong', '{SUPER_PATH}')} d={SUPER_PATH} fill="var(--color-surface-overlay)" stroke={LINE} strokeWidth={sw(1)} />
          ...
          <text {...layer('VesselSynoptic / hull / label.text', 'font/data 9 · ink/muted · small superstructure marker (round 123)', 'SUPER (superstructure marker)')} x={101} y={95} textAnchor="middle" style={mono()} fill={NEUTRAL.inkMuted}>
            <title>SUPERSTRUCTURE</title>SUPER
          </text>
          <text x={26} y={92} textAnchor="middle" style={mono()} fill={NEUTRAL.inkMuted} transform="rotate(-90 26 92)">BOW</text>
          ...
          {[0, 1].map((i) => (
            <line key={`sf${i}`} {...layer('VesselSynoptic / flow / feedLine.line', 'line/strong 1.2px · dashed when no transfer', '{storage→feeder · tank.transfer_active}')} x1={GEOM.storage[i].x + GEOM.storage[i].w} y1={GEOM.storage[i].y + GEOM.storage[i].h / 2}
              ...
              stroke={LINE} strokeWidth={sw(1.2)} strokeDasharray={xferActive ? undefined : '4 3'} />
          ))}
          {[0, 1].map((i) => (
            <line key={`fm${i}`} {...layer('VesselSynoptic / flow / meterLine.line', 'line/strong 1.2px', '{feeder→flow meter}')} x1={GEOM.feeder[i].x + GEOM.feeder[i].w} y1={GEOM.feeder[i].y + GEOM.feeder[i].h / 2}
              x2={GEOM.meter.x} y2={GEOM.meter.y} stroke={LINE} strokeWidth={sw(1.2)} />
          ))}
          {GEOM.engines.map((e) => (
            <line key={`me${e.id}`} {...layer('VesselSynoptic / flow / engineLine.line', 'line/strong 1.2px', '{flow meter→engine}')} x1={GEOM.meter.x} y1={GEOM.meter.y} x2={e.x - e.r} y2={e.y}
              stroke={LINE} strokeWidth={sw(1.2)} />
          ))}
          {xferActive && (
            <text x={292} y={88} textAnchor="middle" style={mono()} fill={INK2}>XFER</text>
          )}
          ...
          {tankGeo.map((g, i) => {
            ...
            return (
              <g key={g.id}>
                <rect {...layer('VesselSynoptic / tanks / tank.shape', 'surface/base · line/strong | TANK_LOW tint (own alert only, round 20)', '{tank.tank_id}')} x={g.x} y={g.y} width={g.w} height={g.h} fill="var(--color-surface-base)" stroke={tint ?? LINE} strokeWidth={sw(tint ? 1.5 : 1)} rx={RADIUS} />
                ...
                <text {...layer('VesselSynoptic / tanks / value.text', 'font/data 11 · ink/primary | tint', '{tank.level_pct}%')} x={g.x + g.w / 2} y={g.y + g.h / 2 + 4} textAnchor="middle" style={mono(CONTEXT)} fill={tint ?? NEUTRAL.ink}>
            ...
            return (
              <g key={g.id}>
                <circle {...layer('VesselSynoptic / engines / node.shape', 'running = filled · stopped = outline · status tint only when alert names the engine', '{engine.engine_id · running}')} cx={g.x} cy={g.y} r={g.r} fill={e.running ? (tint ?? 'var(--color-surface-overlay)') : 'none'}
                  stroke={stroke} strokeWidth={sw(tint ? 2 : 1.2)} />
                <text {...layer('VesselSynoptic / engines / value.text', 'font/data 9', '{engine.load_pct}% | OFF')} x={g.x} y={g.y + 3.5} textAnchor="middle" style={mono()}
                  fill={e.running && tint ? 'var(--color-surface-base)' : INK2}>
          ...
          <rect {...layer('VesselSynoptic / meter / meter.shape', 'surface/overlay · line/strong · 45° diamond', 'flow meter node')} x={GEOM.meter.x - 7} y={GEOM.meter.y - 7} width={14} height={14}
            fill="var(--color-surface-overlay)" stroke={LINE} strokeWidth={sw(1.2)} transform={`rotate(45 ${GEOM.meter.x} ${GEOM.meter.y})`} />
          <text {...layer('VesselSynoptic / meter / value.text', 'font/data 11 · ink/primary', '{flow_gps × 3600} gph')} x={GEOM.meter.x} y={GEOM.meter.y + 28} textAnchor="middle" style={mono(CONTEXT)} fill={NEUTRAL.ink}>
          ...
          <g {...layer('VesselSynoptic / meter / recon.chip', 'border + text = recon severity (OK ink/secondary · advisory · watch)', '{reconciliation.status · error_pct}')}>
            <rect x={GEOM.meter.x - 34} y={GEOM.meter.y - 46} width={68} height={16} rx={RADIUS}
              fill="var(--color-surface-raised)" stroke={reconColor} strokeWidth={sw(1)} />
            <text x={GEOM.meter.x} y={GEOM.meter.y - 34} textAnchor="middle" style={mono(CONTEXT)} fill={reconColor}>
              RECON {recon.status}
            </text>
            <line x1={GEOM.meter.x} y1={GEOM.meter.y - 30} x2={GEOM.meter.x} y2={GEOM.meter.y - 10} stroke={reconColor} strokeWidth={sw(0.75)} />
          </g>
          ...
          {[...tankGeo.map((g) => ({ id: g.id, ax: g.x + g.w / 2, ayTop: g.y, ayBot: g.y + g.h })),
            ...
            return (
              <g key={`co-${c.id}`}>
                <line {...layer('VesselSynoptic / callouts / leader.line', 'line/strong 0.6px', '{node→label}')} x1={c.ax} y1={anchorY} x2={l.lx} y2={l.ly + (up ? 4 : -10)} stroke={LINE} strokeWidth={sw(0.6)} />
                <text {...layer('VesselSynoptic / callouts / label.text', 'font/data 10 · ink/secondary — ST/FD/E annotation language', '{ST1 ST2 FD1 FD2 E1–E4}')} x={l.lx} y={l.ly} textAnchor="middle" style={mono()} fill={INK2}>{c.id}</text>
              </g>
            );
          })}
        </svg>
