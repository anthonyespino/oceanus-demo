...
//
// Motion is JS-phase-driven with CSS opacity transitions (same ease-in-out curve as the build's
// breathe/callout animations). Phases all start at MOUNT — deliberately NOT a CSS keyframe applied
// at SSR, which decouples the animation start (first paint) from the unmount timer (mount) and made
// the visible phase drift run-to-run. Sequence (~2.2s): veil covers solid from first paint (no
// board flash), the logo fades IN, holds, then the whole veil fades OUT to reveal the board, then
// unmounts. prefers-reduced-motion: transitions are removed (CSS) so the mark shows solid then cuts.
//
// The mark is the INLINED oceanus.logo.svg (docs/glyphs-import) — rendered as drawn (white), no
// recolor. Inlined rather than <img> so it has no load/intrinsic-size dependency (an <img> of the
// SVG reported naturalWidth 0 → zero-height render); the viewBox preserves aspect at any width.

import { useEffect, useState } from 'react';

// oceanus.logo.svg — single path, drawn white, viewBox 0 0 100 101 (docs/glyphs-import).
const LOGO_PATH =
...
export function LaunchScreen() {
  const [shown, setShown] = useState(true);        // veil mounted?
  const [revealed, setRevealed] = useState(false); // logo visible?
  const [fadingOut, setFadingOut] = useState(false);

  useEffect(() => {
    const reduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    const rIn = requestAnimationFrame(() => setRevealed(true)); // show the logo (fades in unless reduced)
    if (reduced) {
      const t = setTimeout(() => setShown(false), 700); // brief solid hold, then cut
      return () => { cancelAnimationFrame(rIn); clearTimeout(t); };
    }
    const tOut = setTimeout(() => setFadingOut(true), 1800); // begin the ~0.4s veil fade-out
    const tEnd = setTimeout(() => setShown(false), 2200);    // unmount once faded
  ...
  return (
    ...
    >
      <svg
        ...
        aria-label="Oceanus"
        style={{
          width: '25vw', maxWidth: 280, height: 'auto', display: 'block',
          opacity: revealed ? 1 : 0,
          transition: 'opacity 0.4s ease-in-out',
        }}
      >
        <path fillRule="evenodd" clipRule="evenodd" d={LOGO_PATH} fill="#ffffff" />
      </svg>
    </div>
  );
}
