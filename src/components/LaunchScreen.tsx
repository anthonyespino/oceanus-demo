'use client';
// ROUND 145: CLICK-TO-START splash — foolproof for presenting. The veil shows a static, gently
// pulsing Oceanus mark and WAITS for a click (or Enter/Space); the click runs the water-fill
// load-up, then fades to the board. Why click-to-start: the auto-playing version was fragile on a
// cold load — the fill animation depended on CSS/JS landing before a fixed timeline, and a busy
// first-paint main thread (WebGL + fleet gen + hydration) starved it. Waiting for a click means the
// page is fully loaded + idle when the animation runs, so it plays smoothly EVERY time, and the
// presenter controls when the demo opens.
//
// The veil background uses an inline hex fallback (`var(..., #101010)`) so it's dark from first
// paint even before globals.css loads (no grey flash on a cold cache). The mark is the inlined
// oceanus.logo.svg used as a CSS mask: a dim "empty glass" base, and a white block that rises
// translateY(100%→0) on click (masked → fills bottom-up). Fill + fade are inline transitions —
// reliable because they fire post-click (loaded, main thread free). Logo only; no text/spinner.
// prefers-reduced-motion: no pulse/fill/fade — click cuts straight to the board.
...
type Phase = 'idle' | 'fill' | 'fade' | 'done';

export function LaunchScreen() {
  const [phase, setPhase] = useState<Phase>('idle');

  const start = () => {
    if (phase !== 'idle') return; // ignore extra clicks once running
    const reduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    if (reduced) { setPhase('done'); return; } // straight to the board
    setPhase('fill');
    setTimeout(() => setPhase('fade'), 1700); // fill (1.5s) + brief hold
    setTimeout(() => setPhase('done'), 2150);  // after the veil fade
  };
  ...
  return (
    <div
      ...
      role="button"
      tabIndex={0}
      aria-label="Enter Oceanus Fleet"
      onClick={start}
      onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); start(); } }}
      style={{
        position: 'fixed', inset: 0, zIndex: 1000,
        // hex fallback → dark from first paint even before globals.css loads (no grey on cold cache).
    ...
    >
      {/* the mark, as a mask. Dim "empty glass" base + white block that rises on click. The idle
          mark gently pulses (launch-pulse) to invite the click. */}
      <div
        className={phase === 'idle' ? 'launch-pulse' : undefined}
        style={{
